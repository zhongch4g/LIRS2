...
bool TraceHandle::Init(std::string filename, int32_t cutline)
{
    ...
    // printf("Read all data and store to mem START: %s", ctime(&start_time));
    ...
    while (!feof(mTraceFp))
    {
        ...
        if (dic[ref_blk] > max_page_number)
            max_page_number = dic[ref_blk];
